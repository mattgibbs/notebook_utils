...
class Checklist(object):
	...
	def is_within_limits(self, pv, low_limit, high_limit, description=None):
		return is_within_limits(pv, low_limit, high_limit, description, self)

	def is_below_limit(self, pv, high_limit, description=None):
		return is_below_limit(pv, high_limit, description, self)

	def is_above_limit(self, pv, low_limit, description=None):
		return is_above_limit(pv, low_limit, description, self)

	def is_updating(self, pv, timeout=10.0, description=None):
		return is_updating(pv, timeout, description, self)

	def has_value(self, pv, val, tolerance=None, description=None):
		return has_value(pv, val, tolerance, description, self)
...
def is_updating(pv, timeout=10.0, description=None, checklist=None):
	...
	if checklist is not None:
		if description is None:
			description = pv
		colors = {True: "green", False: "red"}
		msg = {True: "PV is updating", False: "PV not updating"}
...
def has_value(pv, val, tolerance=None, description=None, checklist=None):
	if tolerance is not None:
		return is_within_limits(pv, low_limit=val-tolerance, high_limit=val+tolerance, description=description, checklist=checklist)[1]
	ok = epics.caget(pv) == val
	if checklist is not None:
		if description is None:
			description = pv
		colors = {True: "green", False: "red"}
